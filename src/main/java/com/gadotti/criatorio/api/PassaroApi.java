...
public class PassaroApi {
	...
	public ResponseEntity<?> getPassaroById(@RequestParam Long id) {
		try{
			...
			return new ResponseEntity<Map<String, String>>(Map.of("mensagem", "Erro ao encontrar pássaro"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<?> getAllPassaro() {		
		try {
			...
			return new ResponseEntity<Map<String, String>>(Map.of("mensagem", "Nenhum pássaro encontrado"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	
	@GetMapping("/getAllPassaroBySexoAndEspecieId")
	public ResponseEntity<?> getAllPassaroBySexoAndEspecie(@RequestParam String sexo, @RequestParam("especie_id") Long especieId){
		try {
			List<PassaroDTO> listDTO = passaroService.getAllPassaroBySexoAndEspecieId(sexo, especieId);
			return new ResponseEntity<List<PassaroDTO>>(listDTO, HttpStatus.OK); 
		}catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<Map<String, String>>(Map.of("mensagem", "Nenhum pássaro do sexo "+sexo+" encontrado"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	
	@GetMapping("/getAllPassaroBySexoAndEspecieNome")
	public ResponseEntity<?> getAllPassaroBySexoAndEspecieNome(@RequestParam String sexo, @RequestParam("especie_nome") String especieNome){
		try {
	...
	public ResponseEntity<?> setPassaro(@RequestBody PassaroDTO passaroDTO){
		...
		try{
			...
			return new ResponseEntity<Map<String, String>>(Map.of("mensagem", "Pássaro salvo com sucesso"), HttpStatus.OK);
		}catch(Exception e) {
			e.printStackTrace();
			return new ResponseEntity<Map<String, String>>(Map.of("mensagem","Não foi possível salvar o pássaro. Contultar logs"), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	
	private static boolean checkFields(PassaroDTO dto) {
		if(dto.getNome().isBlank())return false;
		return true;
	}
